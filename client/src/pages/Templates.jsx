  ...
  return (
    <div style={{minHeight:'100vh',overflow:'auto',background:'var(--bg)'}}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',height:58,padding:'0 20px'}}>
          ...
          {isMobile ? (
            ...
            <div style={{display:'flex',gap:5,alignItems:'center'}}>
              {['Templates','Features','Pricing'].map(l=><span key={l} style={{padding:'5px 10px',fontSize:13,color:l==='Templates'?'var(--gold)':'var(--tx3)',cursor:'pointer'}} onClick={()=>{if(l==='Pricing')nav('/pricing');if(l==='Templates')nav('/templates');if(l==='Features')nav('/features')}}>{l}</span>)}
        ...
        {isMobile && navOpen && (
          <div style={{background:'rgba(11,11,13,.98)',borderBottom:'1px solid var(--b1)',boxShadow:'0 8px 32px rgba(0,0,0,.22)',position:'absolute',top:58,left:0,right:0,zIndex:101,animation:'popIn .13s',display:'flex',flexDirection:'column'}}>
            {['Templates','Features','Pricing'].map(l=>(
              <span key={l} style={{padding:'16px 24px',fontSize:16,color:l==='Templates'?'var(--gold)':'var(--tx2)',borderBottom:'1px solid var(--b1)',cursor:'pointer'}} onClick={()=>{setNavOpen(false);if(l==='Pricing')nav('/pricing');if(l==='Templates')nav('/templates');if(l==='Features')nav('/features')}}>{l}</span>
            ))}
      ...
      <section style={{padding:'80px 40px 60px',textAlign:'center'}}>
        ...
        <h1 style={{fontSize:46,lineHeight:1.1,marginBottom:14}}>Browse All Templates</h1>
        <p style={{color:'var(--tx3)',fontSize:16,maxWidth:520,margin:'0 auto'}}>Preview any template for free — no account needed. Pick one and start building.</p>
      </section>
      ...
      <section style={{padding:'0 40px 80px'}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          ...
          <div style={{textAlign:'center',marginTop:24}}><span style={{fontSize:13,color:'var(--tx4)'}}>No account required to preview. Start a free trial to edit and publish.</span></div>
        </div>
      </section>

      {/* CTA */}
      <section style={{padding:'80px 40px',textAlign:'center',background:'var(--bg2)',borderTop:'1px solid var(--b1)'}}>
        <div style={{maxWidth:540,margin:'0 auto'}}>
          <h2 style={{fontSize:38,lineHeight:1.1,marginBottom:16}}>Ready to get started?</h2>
          <p style={{color:'var(--tx3)',fontSize:15,marginBottom:32}}>Start free for 14 days. No credit card required.</p>
